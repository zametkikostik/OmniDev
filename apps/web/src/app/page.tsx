    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100">
      <div className="w-full md:w-[420px] flex flex-col border-r border-zinc-800">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-zinc-800 rounded-2xl px-4 py-3 text-sm text-zinc-400">Генерирую...</div>
            </div>
          )}
          <div ref={bottomRef} />
        </div>
        ...
        <div className="p-4 border-t border-zinc-800">
          ...
          <p className="text-[11px] text-zinc-600 mt-2 text-center">OpenRouter · Ollama · MetaMask billing</p>
        </div>
      </div>
      ...
      <div className="hidden md:flex flex-1 flex-col bg-zinc-900">
        <div className="h-10 border-b border-zinc-800 flex items-center px-4 gap-2">
          ...
          <div className="flex-1 text-center text-xs text-zinc-500 truncate">{previewUrl || 'Превью появится здесь'}</div>
        </div>
        <div className="flex-1 relative">
          {previewUrl ? (
            <iframe src={previewUrl} className="absolute inset-0 w-full h-full border-0" title="OmniDev Preview" allow="cross-origin-isolated" />
          ) : (
            <div className="absolute inset-0 flex items-center justify-center text-zinc-600">
              <div className="text-center">
                <div className="text-5xl mb-4 opacity-30">◈</div>
                <p className="text-sm">Напиши, что хочешь создать</p>
                <p className="text-xs mt-1 text-zinc-700">Например: «SaaS дашборд с авторизацией и Stripe»</p>
              </div>
            </div>
          )}
        </div>
